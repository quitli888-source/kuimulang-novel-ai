...
def delete_env(key: str):
    ...
    lines = [l for l in ENV_FILE.read_text(encoding="utf-8").splitlines()
             if not (l.strip().startswith(f"{key}=") and "=" in l)]
...
DEFAULT_TEMPLATES = [
    WritingTemplate("短篇", 10000, 3, 2500, 4000),
    WritingTemplate("中篇", 30000, 6, 4000, 6000),
    WritingTemplate("长篇", 50000, 10, 4000, 6000),
    WritingTemplate("超长篇", 100000, 20, 4000, 6000),
...
class AgentConfigs:
    ...
    def from_env(cls) -> "AgentConfigs":
        # 从.env读取每个Agent的配置
        cfg = cls()
        for agent in ["plot_planner", "part_writer", "style_optimizer",
...
class AppConfig:
    ...
    confirm_mode: bool = True  # True=手动确认模式，False=自动模式
    ...
    def __post_init__(self):
        ...
        print(f"AppConfig初始化: template={self.template.name}, target_words={self._target_words}, part_count={self._part_count}")

    def apply_template(self, template: WritingTemplate, custom_target_words=None, custom_part_count=None):
        self.template = template
        if template.name == "自定义":
            # 自定义模板使用提供的值或从.env读取的值
            if custom_target_words and custom_part_count:
                ...
                if custom_target_words and custom_part_count:
                    ...
                    self._part_word_min = max(1000, self._target_words // self._part_count // 2)
                    self._part_word_max = min(10000, self._target_words // self._part_count * 2)
            print(f"应用自定义模板: target_words={self._target_words}, part_count={self._part_count}")
            ...
            print(f"应用预设模板 {template.name}: target_words={self._target_words}, part_count={self._part_count}")
...
# V4兼容导出（供 from config import PART_COUNT 使用）
...
def get_llm_config_for_agent(agent_name: str) -> LLMConfig:
    ...
    if provider is None:
        print(f"[Config] 警告: 供应商 '{provider_id}' 未找到，使用默认供应商 minimax")
        provider = get_provider_from_id("minimax")

    # 从.env加载对应供应商的API Key
    loaded = load_provider_api_key(provider, read_env)
    
    # 检查API Key是否有效
    if not loaded.api_key:
        print(f"[Config] 错误: 供应商 '{provider_id}' 的 API Key 未配置!")
        print(f"[Config] 请在 .env 文件中设置 {loaded.api_key_name}")
    else:
        key_preview = loaded.api_key[:10] + "..." if len(loaded.api_key) > 10 else "[空]"
        print(f"[Config] 供应商 '{provider_id}' API Key 已配置: {key_preview}")

    return LLMConfig(
        api_key=loaded.api_key,
        base_url=loaded.base_url,
        model=loaded.model,
        json_model=loaded.json_model,
    )
...
def get_all_providers() -> list:
    ...
    for p in PRESET_PROVIDERS:
        ...
        result.append({
            "id": loaded.id,
            "name": loaded.name,
            "has_api_key": bool(loaded.api_key and loaded.api_key != ""),
            "base_url": loaded.base_url,
            "model": loaded.model,
            "json_model": loaded.json_model,
        })

...
logger = get_logger('test_r7_full_e2e')
...
def call_judge_llm(system: str, user: str, max_tokens: int=1500) -> str:
    ...
    raw = _original_call_llm(system_prompt=system, user_prompt=user, temperature=0.2, max_tokens=max_tokens, agent='test_r7_judge', stream=False)
    text = raw.strip()
    if text.startswith('```'):
        text = text.strip('`')
...
def main():
    logger.info('=' * 70)
    logger.info('【R7-T7 / T8】完整 E2E Part 1-3 + 3 Review Agents + 评分')
    logger.info('=' * 70)
    ...
    part_outline = [{'phase': '开端', 'title': '雨夜重逢', 'core_event': '林枫推开尘封的书房门', 'emotion_target': '悬疑与不安', 'key_dialogue': '你终于来了', 'end_hook': '墙上红字指向下一个地点', 'pacing': '慢起', 'causality': '承前：搭档失踪悬案', 'word_count': 1500}, {'phase': '发展', 'title': '墨迹追凶', 'core_event': '林枫解读红字暗号', 'emotion_target': '紧张', 'key_dialogue': '这不是失踪，是潜伏', 'end_hook': '暗号指向钟楼', 'pacing': '中等', 'causality': '承 Part1 红字', 'word_count': 1500}, {'phase': '高潮', 'title': '钟楼对峙', 'core_event': '林枫找到沈渊', 'emotion_target': '震撼', 'key_dialogue': '三年，你终于来了', 'end_hook': '组织已发现他们', 'pacing': '快', 'causality': '承 Part2 暗号', 'word_count': 1500}]
    ...
    for part_num in (1, 2, 3):
        ...
        logger.info(f"  Part {part_num}: {len(text)} 字, {result.get('chunk_count')} chunk, {dt:.1f}s")
        all_results.append({'part': part_num, 'text': text, 'words': len(text), 'chunks': result.get('chunk_count'), 'elapsed_s': dt})
    review_results = []
    ...
    for part_data in all_results:
        ...
        for name, agent in [('logic', logic_agent), ('emotion', emotion_agent), ('consistency', consistency_agent)]:
            ...
            try:
                ...
                dt = time.time() - t0
                logger.info(f"  {name}: OK ({dt:.1f}s) | keys={(list(r.keys())[:5] if isinstance(r, dict) else 'N/A')}")
                per_part[name] = r
            except Exception as e:
                logger.info(f'  {name}: FAIL {type(e).__name__}: {e}')
                per_part[name] = {'error': str(e), 'pass': False}
        review_results.append(per_part)
    summary = tracker.get_summary()
    logger.info(f'\n========== CostTracker ==========')
    logger.info(f"  total_calls = {summary['total_calls']}")
    logger.info(f"  total_tokens = {summary['total_tokens']} (in={summary['prompt_tokens']}, out={summary['completion_tokens']})")
    logger.info(f"  estimated_cost_rmb = ¥{summary['estimated_cost_rmb']}")
    logger.info(f"  by_model = {summary['model_breakdown']}")
    logger.info(f'  checkpoints fired = {len(checkpoint_log)}')
    logger.info(f'\n========== T8 Scoring (LLM-as-judge) ==========')
    parts_combined = '\n\n'.join((f"=== Part {pd['part']} ===\n{pd['text']}" for pd in all_results))
    logger.info(f'  Combined parts length = {len(parts_combined)}')
    logger.info('\n--- 人物一致性评分 ---')
    consistency_score_payload = call_judge_llm(system='你是中文小说编辑，专精角色一致性评估。我会给你 3 个 Part 的完整正文，请你判断两个核心角色（林枫、沈渊）在跨 Part 中的核心特质是否一致。请用 JSON 输出：{"linfeng_consistency": 1-10, "shenyuan_consistency": 1-10, "overall_score": 1-10, "analysis": "200字以内理由"}。', user=f"以下 3 个 Part 的角色表现是否前后一致？特别是'林枫'在 Part 1 是执拗敏锐的前刑警，Part 3 是否仍如此？'沈渊'在 Part 2 是潜伏的搭档，Part 3 重逢时性格是否一致？\n\n{parts_combined[:12000]}")
    logger.info(f'  人物一致性评分 (raw): {consistency_score_payload}')
    char_consistency = consistency_score_payload.get('overall_score', None) if isinstance(consistency_score_payload, dict) else None
    logger.info('\n--- 情节连贯性评分 ---')
    coherence_score_payload = call_judge_llm(system='你是中文小说编辑，专精剧情连贯性评估。请检查 3 个 Part 之间的剧情衔接。1) Part 1 末尾的\'钩子\'（红字暗号指向下一地点）是否被 Part 2 承接？2) Part 2 末尾的\'暗号指向钟楼\'是否被 Part 3 实现？3) 是否有\'前文 X 事件\'被 Part N 突然否定或遗忘？请用 JSON 输出：{"hook_carried": 0/1, "hook_score": 1-10, "clock_tower_present": 0/1, "clock_score": 1-10, "no_contradictions": 0/1, "overall_score": 1-10, "analysis": "200字以内理由"}。', user=f'请评估 3 个 Part 的情节连贯性。\n\n{parts_combined[:12000]}')
    logger.info(f'  情节连贯性评分 (raw): {coherence_score_payload}')
    coherence_score = coherence_score_payload.get('overall_score', None) if isinstance(coherence_score_payload, dict) else None
    logger.info('\n--- 可读性评分 ---')
    readability_payload = call_judge_llm(system='你是中文小说编辑，专精可读性评估。请评估 3 个 Part 的整体可读性：中文流畅度、节奏感、信息密度、感官描写、对白占比。请用 JSON 输出：{"fluency": 1-10, "pacing": 1-10, "density": 1-10, "sensory": 1-10, "overall_score": 1-10, "analysis": "200字以内理由"}。', user=f'请评估以下 3 个 Part 的可读性。\n\n{parts_combined[:12000]}')
    logger.info(f'  可读性评分 (raw): {readability_payload}')
    readability_score = readability_payload.get('overall_score', None) if isinstance(readability_payload, dict) else None
    out_md_path = os.path.join(os.path.abspath(os.path.join(ROOT, '..', '.reviews', 'round_7')), 'test.md')
    ...
    with open(out_md_path, 'w', encoding='utf-8') as f:
        f.write('\n'.join(lines))
    logger.info(f'\nWrote {out_md_path}')
    logger.info(f"  Part 1-3 长度: {[pd['words'] for pd in all_results]}")

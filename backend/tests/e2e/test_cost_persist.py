...
def test_record_increments_calls():
    ...
    assert "calls" in summary0  # R5-P0-3: get_summary 返回值含 calls 列表
    ...
    print("[test_record] PASS: record 3 次后 calls=3, summary.calls=3")
...
def test_attach_work_loads_history_from_summary():
    ...
    # 1. 旧进程：record 3 次 → get_summary() 拿到含 calls 的字典
    ...
    # 2. 新进程：新 CostTracker 实例 in-memory 起点 0
    new_tracker = CostTracker()
    assert len(new_tracker.calls) == 0

    # 3. attach_work(work_id, persisted) → self.calls 应恢复为 3 条
    new_tracker.attach_work("work_history", persisted)
    assert len(new_tracker.calls) == 3, (
        f"attach_work 应恢复 3 条历史，实际 {len(new_tracker.calls)}"
    )
    print("[test_attach_load] PASS: 新 tracker attach_work 后 calls=3")
...
def test_attach_work_dedup():
    ...
    # 二次 attach 同一份 summary（模拟重启后重新读 works.json 里同一条 cost_summary）
    tracker.attach_work("work_dedup", snapshot)
    assert len(tracker.calls) == 2, (
        f"重复 attach 应去重，实际 {len(tracker.calls)}"
    )
    print("[test_attach_dedup] PASS: 重复 attach 后仍 calls=2")
...
def test_restart_simulation_record_3_attach_3_then_record_1():
    ...
    assert len(new_tracker.calls) == 4, (
        f"重启合并 + 增量后应有 4 条，实际 {len(new_tracker.calls)}"
    )

    summary = new_tracker.get_summary()
    assert summary["total_calls"] == 4
    assert len(summary["calls"]) == 4
    print(f"[test_restart_merge] PASS: record 3 + attach 3 + record 1 = calls={len(new_tracker.calls)}")
...
def test_get_summary_calls_field():
    ...
    print("[test_get_summary_calls] PASS: get_summary() 返回值含 'calls' 字段")


# ---- 直接 python 跑 ----
if __name__ == "__main__":
    print("=" * 60)
    print("test_cost_persist.py —— R5-P3-5.3")
    print("=" * 60)

    tests = [
        ("record increments", test_record_increments_calls),
        ("attach loads history", test_attach_work_loads_history_from_summary),
        ("attach dedup", test_attach_work_dedup),
        ("restart merge", test_restart_simulation_record_3_attach_3_then_record_1),
        ("get_summary.calls field", test_get_summary_calls_field),
    ]

...
async def main():
    ...
    logger.info(f'[smoke] tmpdir = {tmpdir}')
    ...
    cfg_mod._LLM_CONFIG_FILE.write_text(json.dumps({'active_provider_id': 'step', 'per_agent_enabled': False, 'agent_providers': {}, 'global_temperature': 0.7, 'agent_temperatures': {}}), encoding='utf-8')
    part_text = '这是第一段。\n这是第二段。\n' * 200
    ...
    work_id = 'smoke_001'
    work_data = {'work_id': work_id, 'title': 'Smoke 测试作品', 'user_theme': '雨夜的约定', 'config': {'template': '短篇', 'part_count': 3, 'target_words': 10000, 'part_word_min': 2500, 'part_word_max': 4000, 'confirm_mode': False}, 'phase': 'init', 'parts': {}, 'part_summaries': {}, 'cost_summary': {'calls': []}, 'foreshadowing': [], 'character_state_track': {}, 'part_outline': [{'title': f'Part {i + 1}', 'core_event': f'event{i + 1}', 'emotion_target': 'tension', 'key_dialogue': 'dialog', 'end_hook': 'hook', 'pacing': 'normal', 'causality': 'c', 'phase': 'setup', 'word_count': 3000} for i in range(3)]}
    work_file = cfg_mod.WORKS_DIR / f'{work_id}.json'
    work_file.write_text(json.dumps(work_data, ensure_ascii=False, indent=2), encoding='utf-8')
    ...
    logger.info('[smoke] 启动 WritingService.run()')
    t0 = time.time()
    try:
        await asyncio.wait_for(service.run(), timeout=120)
    except asyncio.TimeoutError:
        logger.info('[smoke] TIMEOUT after 120s — 部分流程可能未完成')
    logger.info(f'[smoke] run() 返回, 耗时 {time.time() - t0:.1f}s')
    saved = json.loads(work_file.read_text(encoding='utf-8'))
    logger.info(f"[smoke] phase = {saved.get('phase')}")
    logger.info(f"[smoke] parts count = {len(saved.get('parts', {}))}")
    logger.info(f"[smoke] part_summaries count = {len(saved.get('part_summaries', {}))}")
    cs = saved.get('cost_summary') or {}
    logger.info(f'[smoke] cost_summary keys = {list(cs.keys())}')
    logger.info(f"[smoke] cost_summary.calls 长度 = {len(cs.get('calls', []))}")
    logger.info(f"[smoke] cost_summary.total_cost_rmb = {cs.get('total_cost_rmb')}")
    phase_ok = saved.get('phase') in ('phase3_part4', 'phase4', 'phase5_review', 'complete')
    parts_ok = len(saved.get('parts', {})) >= 1
    cost_ok = 'calls' in cs
    logger.info()
    logger.info(f'[smoke] phase_ok={phase_ok} parts_ok={parts_ok} cost_ok={cost_ok}')
    phase_events = [l for l in fake_emitter.logs if 'PHASE' in l[0]]
    logger.info(f'[smoke] PHASE 事件数: {len(phase_events)}')
    for ev in phase_events[:10]:
        logger.info(f"  - {ev[0]}: {ev[1].get('phase', ev[1].get('name', '?'))}")
    import shutil

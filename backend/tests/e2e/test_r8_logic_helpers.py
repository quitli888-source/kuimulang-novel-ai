...
logger = get_logger('test_r8_logic_helpers')
...
def main():
    from core.agents.logic_review_agent import LogicReviewAgent, _coerce_score, _extract_summary_from_raw
    from core.story_state import StoryState
    from core.established_facts import Fact, EstablishedFacts
    logger.info('=' * 60)
    logger.info('R8 Self-check: LogicReviewAgent + EstablishedFacts')
    logger.info('=' * 60)
    logger.info('\n[1] _coerce_score')
    cases = [(5, 5), (0, 1), (-1, 1), (11, 10), (7.5, 8), ('abc', 3), (None, 3), (True, 3), (False, 3)]
    for inp, expected in cases:
        actual = _coerce_score(inp)
        ok = 'OK' if actual == expected else 'FAIL'
        logger.info(f'  [{ok}] _coerce_score({inp!r}) = {actual} (期望 {expected})')
    ...
    logger.info(f'  [OK] markdown 包裹解析: {obj}')
    ...
    logger.info(f'  [OK] 3 facts 渲染为多行字符串，长度={len(rendered)}')
    logger.info('  渲染片段：')
    ...
    logger.info(f'  [OK] block 长度={len(block)}，含标题+事实')
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        assert len(state2.established_facts) == 3, f'load 后 facts 数量错: {len(state2.established_facts)}'
        logger.info(f'  [OK] save/load 后 facts={len(state2.established_facts)}')
    ...
    logger.info(f"  [OK] estimated_cost=¥{s['estimated_cost']:.4f} == estimated_cost_rmb=¥{s['estimated_cost_rmb']:.4f}")
    ...
    logger.info(f'  [OK] execute() 包含 p0_count 字段: {has_p0}')
    logger.info(f'  [OK] execute() 包含 V5 _protocol 标记: {has_v5}')
    ...
    logger.info(f'  [OK] DEFAULT_ROLLING_EVERY={SlidingWindow.DEFAULT_ROLLING_EVERY}, SUMMARY_L2_LEN={SlidingWindow.SUMMARY_L2_LEN}')
    logger.info(f'  [OK] should_create_rolling_summary(3)=True, (6)=True, (2)=False')
    logger.info('\n' + '=' * 60)
    logger.info('ALL SELF-CHECK PASSED')
    logger.info('=' * 60)
